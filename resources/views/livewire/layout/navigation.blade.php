...
<nav x-data="{ open: false }" class="glass border-b border-white/10 relative z-50">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" wire:navigate>
                        <x-application-logo class="block h-9 w-auto fill-current text-indigo-400 filter drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]" />
                    </a>
                </div>

                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" wire:navigate class="text-gray-300 hover:text-white transition-colors">
                        {{ __('Dashboard') }}
                    </x-nav-link>

                    <x-nav-link :href="route('tasks')" :active="request()->routeIs('tasks')" wire:navigate class="text-gray-300 hover:text-white transition-colors">
                        {{ __('Tasks') }}
                    </x-nav-link>
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-4 py-2 glass hover:bg-white/10 border border-white/5 text-sm leading-4 font-bold rounded-xl text-gray-200 transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden glass-dark border-t border-white/5">
        ...
        <div class="pt-4 pb-1 border-t border-white/5">
            <div class="px-4">
                ...
                <div class="font-medium text-sm text-indigo-300/50">{{ auth()->user()->email }}</div>
            </div>
